...
function verify_parameter() {
    ...
    if [[ "$DEST_DIR" == "" ]]; then
        echo -e "$TIMESTAMP - \e[00;31mparameter not found, sh transfer.install.sh <source_ip> <service_name> <dest_ip> <dest_dir>\e[00m"
        exit -1
    fi
}
...
function get_mod() {
    ...
    if [[ ! -f "freelogin.sh" ]]; then
        ...
        wget http://$SERVER_IP:8082/shared/devops/utils/freelogin.sh
    else
...
function transfer() {
    ...
    if [[ "$SOURCE_IP" == "127.0.0.1" ]] && [[ "$SOURCE_IP" == "$DEST_IP" ]]; then
        ...
        if [[ ! "$SOURCE_DIR" =~ "/" ]]; then
            ...
            SOURCE_DIR=${SOURCE_DIR%/*}
            if [[ "$SOURCE_DIR" == "" ]]; then
                echo -e "$TIMESTAMP - \e[00;31m$SERVICE_NAME srvice process not found!\e[00m"
                exit -1
            fi
        fi
        echo -e "\e[00;33mSOURCE_DIR:$SOURCE_DIR\e[00m"
        if [[ ! -d "$SOURCE_DIR" ]]; then
            echo -e "$TIMESTAMP - \e[00;31m$SOURCE_DIR dir not found!\e[00m"
            exit -1
        fi
        echo -e "$TIMESTAMP - transfer from $SOURCE_DIR to $DEST_DIR/$SERVICE_NAME ..."
        ...
        ./service.sh stop `ls *.jar`
        rm -rf apilogs catlina.out
        echo -e "$TIMESTAMP - transfer from $SOURCE_DIR to $DEST_IP:$DEST_DIR/$SERVICE_NAME ..."
        ...
        echo -e "$TIMESTAMP - not support this case!"
        exit -1
